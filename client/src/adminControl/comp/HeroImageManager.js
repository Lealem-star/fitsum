...
const HeroImageManager = () => {
  ...
  const fetchImages = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/home-hero/admin/images');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingImage) {
        await api.put(`/api/home-hero/admin/images/${editingImage._id}`, submitData, {
          headers: {
  ...
  if (loading) {
    return <div className="text-center py-10 text-gray-600 text-xl">Loading hero images...</div>;
  }
      ...
      {showForm && (
        <div className="bg-gray-50 p-6 rounded-lg mb-8 border border-gray-200">
          <form onSubmit={handleSubmit} encType="multipart/form-data">
            <h3 className="mt-0 mb-5 text-gray-800 text-2xl">
                ...
                {previewUrl && (
                  <div className="mt-4 text-center">
                    <img
                      src={previewUrl}
                      alt="Preview"
                      className="max-w-full max-h-[300px] rounded-lg shadow-md mt-2.5 mx-auto"
                    />
                  </div>
                )}
                {editingImage && !selectedFile && (
                  <div className="mt-4 p-4 bg-blue-50 rounded border-l-4 border-[#61dafb]">
                    <p className="m-0 mb-2.5 text-gray-600 text-sm">
                      Current image will be kept if no new file is selected.
                    </p>
                    <img
                      src={editingImage.imageUrl}
                      alt="Current"
                      className="max-w-full max-h-[200px] rounded mt-2.5"
                    />
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-2.5 mt-6">
              ...
              <button
                type="button"
                className="px-5 py-2.5 border-none rounded cursor-pointer text-base transition-all duration-300 bg-gray-500 text-white hover:bg-gray-600"
        ...
        {images.length === 0 ? (
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5 md:grid-cols-[repeat(auto-fill,minmax(300px,1fr))] grid-cols-1">
            {images.map((image) => (
              <div
                key={image._id}
                className={`bg-white border border-gray-200 rounded-lg overflow-hidden transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-md ${
              ...
              >
                <div className="relative w-full h-[200px] overflow-hidden bg-gray-100">
                  ...
                  />
                  {!image.isActive && (
                    <div className="absolute top-2.5 right-2.5 bg-red-600 text-white px-2.5 py-1.5 rounded text-xs font-bold">
                      Inactive
                    </div>
                  )}
                </div>
                <div className="p-4">
                  ...
                  <p className="text-gray-400 text-xs m-0 break-all font-mono">
                    {image.imageUrl}
                  </p>
                </div>
